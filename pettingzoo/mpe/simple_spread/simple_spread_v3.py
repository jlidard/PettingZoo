...
class raw_env(SimpleEnv, EzPickle):
    def __init__(
        ...
        scenario = Scenario()
        world = scenario.make_world(N, params)
        super().__init__(
            scenario=scenario,
            world=world,
...
class Scenario(BaseScenario):
    def make_world(self, N=3, params=None):
        world = World()
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            agent.silent = True
            agent.size = params["agent_size"] if params is not False else 0.1
            agent.population_id = np.random.choice(2, 1)
        # add landmarks
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            landmark.size = params["landmark_size"] if params is not False else 0.05
        return world

    def reset_world(self, world, np_random):
        # random properties for agents
        for i, agent in enumerate(world.agents):
            agent.color = np.array([0.35, 0.35, 0.85]) if world.params is False else np.array(world.params[f"agent_color_pop_{agent.population_id[0]}"]) #
        # random properties for landmarks
        for i, landmark in enumerate(world.landmarks):
            landmark.color = np.array([1, 0.25, 0.25])
        # set random initial states
        for agent in world.agents:
            agent.state.p_pos = np_random.uniform(-1, +1, world.dim_p) * np.sqrt(len(world.agents))
            agent.state.p_vel = np.zeros(world.dim_p)
            agent.state.c = np.zeros(world.dim_c)
        for i, landmark in enumerate(world.landmarks):
            landmark.state.p_pos = np_random.uniform(-1, +1, world.dim_p) * np.sqrt(len(world.agents))
